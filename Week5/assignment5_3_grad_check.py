import numpy as np
from Week5.testCases_2 import *
from Week5.gc_utils import sigmoid, relu, dictionary_to_vector, vector_to_dictionary, gradients_to_vector
...
def gradient_check(x, theta, epsilon=1e-7):
    ...
    """
    # Compute gradapprox using left side if formula(1).epsilon is small enough, you don't need to worry about the limit
    ...
    diffetence = numerator / denominator
...
def forward_propagation_n(X, Y, parameters):
    ...
    # Cost
    cost = -1. / m * np.sum(np.dot(np.log(A3), Y.T) + np.dot(np.log(1 - A3), (1 - Y.T)))
